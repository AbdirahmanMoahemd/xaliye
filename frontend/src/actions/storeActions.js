...
export const createNewStoreItem =
  ...
  export const updateStoreItemCountInStock = (id, countInStock) => async (dispatch, getState) => {
    try {
      dispatch({
        type: STORE_UPDATE_REQUEST,
      });
  ...
  export const listStoreItems = (keyword = "") => async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.get(`/api/store?keyword=${keyword}`, config);
